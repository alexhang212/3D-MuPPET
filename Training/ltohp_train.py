...
# from mvn.datasets import human36
from mvn.datasets import utils as dataset_utils

from Utils import MultiPigeon3D_Dataset, Custom_vis
...
def parse_args():
    ...
    parser.add_argument("--config", type=str, required=True, default = "Configs/ltohp_pigeonConfig.yaml", help="Path, where config file is stored")
    parser.add_argument('--eval', action='store_true', help="If set, then only evaluation will be done")
    parser.add_argument('--eval_dataset', type=str, default='train', help="Dataset split on which evaluate. Can be 'train' and 'val'")

    parser.add_argument("--local_rank", type=int, help="Local rank of the process on the node")
    parser.add_argument("--seed", type=int, default=42, help="Random seed for reproducibility")

    parser.add_argument("--logdir", type=str, default="./Weights/ltohp_Weights", help="Path, where logs will be stored")

    args = parser.parse_args()
    return args


def setup_3dpop_dataloaders(config, is_train, distributed_train):
    train_dataloader = None
    if is_train:
        # train
        train_dataset = MultiPigeon3D_Dataset.MultiPigeon3D_Dataset(DatasetPath = config.dataset["train"]["root"],
                                             JSONPath = config.dataset["train"]["labels_path"],
                                             Type = "Train", cuboid_side=config.model.cuboid_side)
    ...
    # val
    val_dataset =MultiPigeon3D_Dataset.MultiPigeon3D_Dataset(DatasetPath = config.dataset["val"]["root"],
                                             JSONPath = config.dataset["val"]["labels_path"],
                                             Type = "Val")
...
def setup_dataloaders(config, is_train=True, distributed_train=False):
    if config.dataset.kind == "3DPOP":
        train_dataloader, val_dataloader, train_sampler = setup_3dpop_dataloaders(config, is_train, distributed_train)
...
def one_epoch(model, criterion, opt, config, dataloader, device, epoch, n_iters_total=0, is_train=True, caption='', master=False, experiment_dir=None, writer=None):
                ...
                if master:
                    if n_iters_total % config.vis_freq == 0:# or total_l2.item() > 500.0:
                        ...
                        for batch_i in range(min(batch_size, config.vis_n_elements)):
                            keypoints_vis = Custom_vis.visualize_batch(
                            ...
                            heatmaps_vis = Custom_vis.visualize_heatmaps(
    ...
    return n_iters_total, metric_dict
...
def main(args):
    ...
    if not args.eval:
        ...
        for epoch in range(config.opt.n_epochs):
            ...
            n_iters_total_train,metric_dict_train = one_epoch(model, criterion, opt, config, train_dataloader, device, epoch, n_iters_total=n_iters_total_train, is_train=True, master=master, experiment_dir=experiment_dir, writer=writer)
            n_iters_total_val,metric_dict_val = one_epoch(model, criterion, opt, config, val_dataloader, device, epoch, n_iters_total=n_iters_total_val, is_train=False, master=master, experiment_dir=experiment_dir, writer=writer)
            # import ipdb;ipdb.set_trace()

            ###
            CurrentLoss = np.mean(metric_dict_val["total_loss"])
            if CurrentLoss < LowestLoss:
                
                if os.path.exists(LowestSavePath):
                    shutil.rmtree(LowestSavePath) #remove previous one


                LowestLoss = CurrentLoss.copy()
                Lowest_checkpoint_dir = os.path.join(experiment_dir, "checkpoints", "Lowest_E{:04}".format(epoch))
                os.makedirs(Lowest_checkpoint_dir, exist_ok=True)

                torch.save(model.state_dict(), os.path.join(Lowest_checkpoint_dir, "weights.pth"))
